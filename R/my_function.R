  ...
  for(i in 1:ncol(ds)) {
    ...
    if (task=="regression") {
      ...
      for (cname in class.name) {
        ...
        base <- pred.all==cl & cl==cname

        idx1 <- which(pred.F1 != cl & pred.other != cl & base)
        f_interact[cnt] <- length(idx1) / N

        idx2 <- which(pred.F1 == cl & pred.other == cl & base)
        f_contribute_common[cnt] <- length(idx2) / N

        idx3 <- which(pred.F1 == cl & pred.other != cl & base)
        f_contribute_F1[cnt] <- length(idx3) / N

        idx4 <- which(pred.F1 != cl & pred.other == cl & base)
...
###########################################################################
one_FeatureInt <- function(model, train, target.name, F1, F2, seed=100, task="regression", class=NULL) {
  ...
  #for(i in 1:ncol(ds)) {
  {
    ...
    if (task=="regression") {
      ...
      base <- pred.F1F2 == cl & pred.all==cl & cl==class

      idx1 <- which(pred.F1 != cl & pred.F2 != cl & base )
      f_interact <- length(idx1) / N

      idx2 <- which(pred.F1 == cl & pred.F2 == cl & base )
      f_contribute_common <- length(idx2) / N

      idx3 <- which(pred.F1 == cl & pred.F2 != cl & base )
      f_contribute_F1 <- length(idx3) / N

      idx4 <- which(pred.F1 != cl & pred.F2 == cl & base )
...
#' @import parallel
#' @import doParallel
pair_FeatureInteract <- function(model, train, target.name, F1, F2, itr=10, task="regression", class=NULL, parallel=F, pkg=NULL) {

  result <- one_FeatureInt(model, train, target.name, F1, F2, seed=100, task=task, class=class)

  if (itr >=2) {
    if(!parallel) {
          for (i in 2:itr) {
            tmp <- one_FeatureInt(model, train, target.name, F1, F2, seed=100+i, task=task, class=class)
          ...
          for (i in 1:length(final)) {
            result$interact <- result$interact + final[[i]]$interact
            result$cont_F1  <- result$cont_F1 + final[[i]]$cont_F1
            result$cont_F2  <- result$cont_F2 + final[[i]]$cont_F2
            result$cont_common <- result$cont_common + final[[i]]$cont_common
          }
...
generate.data <- function(model, train, target.name, F1, F2, itr, task) {
  ...
  if (task == "regression") {
    ...
    base <- (pred.all== cl & pred.F1F2[,itr+1]==cl)
